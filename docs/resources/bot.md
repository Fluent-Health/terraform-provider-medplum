...
- `cron_string` (String) Cron schedule for the bot, as a standard 5-field expression (minute hour day-of-month month day-of-week), evaluated in UTC — e.g. `0 2 * * *` for 02:00 daily. Requires the `cron` feature enabled on the Medplum Project; without it the schedule is stored on the Bot but never runs. An invalid expression is rejected at plan time.
- `description` (String)
